...
./build_benzhi_docker.sh benzhi-task-270-amd64 linux/amd64
./build_benzhi_docker.sh benzhi-task-270-arm64 linux/arm64
docker run -it benzhi-task-270-amd64:latest
docker run -it --platform linux/arm64 benzhi-task-270-arm64:latest
```

## 题目验证命令

1. 预期退出码 0：`go test ./internal/warehouse -run '^TestTask0008CustomsRejectionRollbackKeepsSlot$' -count=1`
2. 预期退出码 0：`go test ./...`
