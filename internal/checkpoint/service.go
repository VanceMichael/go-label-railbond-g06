...
func (s Service) RecordScan(ctx context.Context, u domain.User, consignmentID, checkpointID, scanner, evidence, requestID string) (Scan, error) {
	ctx = checkpointPersistenceContext(ctx)
	...
	err := s.Store.WithTx(ctx, func(tx *storage.Tx) error {
		...
		if _, err := tx.Exec(ctx, "UPDATE consignments SET status='at_checkpoint',current_checkpoint=?,version=version+1 WHERE tenant_id=? AND id=? AND current_checkpoint<?", seq, u.TenantID, consignmentID, seq); err != nil {
			return err
		}
