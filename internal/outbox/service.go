...
func (s Service) Fail(ctx context.Context, u domain.User, id, owner string, epoch int, reason string) error {
	res, err := s.Store.Exec(ctx, "UPDATE outbox_messages SET status='pending',lease_owner=NULL,lease_until=NULL,last_error=?,available_at=? WHERE tenant_id=? AND id=? AND status='sending' AND lease_owner=? AND lease_epoch=?", reason, time.Now().UTC().Add(time.Minute).Format(time.RFC3339Nano), u.TenantID, id, owner, epoch)
	if err != nil {
		return err
	}
