...
func (s TimelineRepository) Load(ctx context.Context, u domain.User, id string) ([]TimelineEntry, error) {
	rows, err := s.Store.Query(ctx, "SELECT 'checkpoint',e.evidence_hash,e.scanner_id FROM checkpoint_events e JOIN consignments c ON c.id=e.consignment_id WHERE e.tenant_id=? AND c.tenant_id=? AND c.id=? ORDER BY e.created_at", u.TenantID, u.TenantID, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
