...
func (s Service) Move(ctx context.Context, u domain.User, from, to, consignmentID, requestID string) error {
	return s.Store.WithTx(ctx, func(tx *storage.Tx) error {
		...
		if err := tx.QueryRow(ctx, "SELECT status FROM warehouse_slots WHERE tenant_id=? AND id=?", u.TenantID, from).Scan(&fromStatus); err != nil {
			return err
		}
		if err := tx.QueryRow(ctx, "SELECT status FROM warehouse_slots WHERE tenant_id=? AND id=?", u.TenantID, to).Scan(&toStatus); err != nil {
			return err
		}
